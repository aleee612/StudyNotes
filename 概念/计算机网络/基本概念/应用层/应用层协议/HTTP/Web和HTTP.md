...
- 第一行：请求行，包含方法字段，URL字段，HTTP版本字段
	- 方法字段：[[方法字段#GET|GET]]、[[方法字段#POST|POST]]、HEAD、PUT、DELETE；获取资源时最常见的是 GET
	- URL：请求的对象标识（对象URL）
- 第二行+：首部行
	- `Host`：服务器主机位置
	- `Connection`：是否持续
...
- GET 请求通常不使用消息体；POST 常用消息体提交数据，但“只有 POST 才能有请求体”并不是 HTTP 的通用语法规则
#### 响应报文
